...
use rand::{self, Rng};
use raylib::{color::Color, ffi::{Rectangle, Vector2}, prelude::{RaylibDraw, RaylibDrawHandle}};
        ...
        for object in self.organisms.iter() {
            match object.behaviour_type {
                BehaviourArchetypes::Prey => {
                    d.draw_rectangle_pro(Rectangle {x: object.pos.x as f32, y: object.pos.y as f32, width: 5.0, height: 5.0 },Vector2 {x:0.0, y: 0.0}, 0.0, Color::GREEN);

                },
                BehaviourArchetypes::Hunter => {
                    d.draw_rectangle_pro(Rectangle {x: object.pos.x as f32, y: object.pos.y as f32, width: 5.0, height: 5.0 },Vector2 {x:0.0, y: 0.0}, 0.0, Color::RED);
                },
            }
        }
    }
...
impl Organism {
    pub fn new(metabolism: f64, speed: f64, vision: i32, pos: DVec2, behaviour_type: BehaviourArchetypes ) -> Self {
        Self { energy: 100.0, metabolism, speed, vision, pos, behaviour_type, current_behaviour: CurrentBehaviour::Idle, visible_creatures: vec![] }
    }
        ...
        match self.current_behaviour {
            CurrentBehaviour::Active(flee_dir) => {
                match self.behaviour_type {
                    BehaviourArchetypes::Prey => {
                        self.pos -= flee_dir.normalize() * movement
                    },
                    BehaviourArchetypes::Hunter => {
                        self.pos += flee_dir.normalize() * 1.0
                    },
                }
                ;
            },
    ...
    fn vision(&mut self) {
        match self.behaviour_type {
            ...
            BehaviourArchetypes::Hunter => {
                let closest_prey: Option<DVec2> = self.visible_creatures.iter().min_by(|a, b| a.distance(self.pos).total_cmp(&b.distance(self.pos))).copied();
